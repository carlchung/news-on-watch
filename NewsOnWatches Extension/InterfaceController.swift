...
class InterfaceController: WKInterfaceController, WCSessionDelegate {
    ...
    func getFeedSettingFromParent() {
        ...
        session.sendMessage(openParentDict, replyHandler: {( replyDictionary: [String : AnyObject]) -> Void in
            // handle reply from iPhone app here
            print("session.sendMessage replyHandler")
            print(replyDictionary["message"]);
            ...
            if let castedResponseDictionary = replyDictionary as? [String: [String]]
            {
                ...
                var i : Int
                for i = 0 ; i < responseMessage.count; i += 1 {
